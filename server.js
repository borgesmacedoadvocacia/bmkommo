...
<head>
  ...
  /* Central BM — o acesso a este dashboard agora é pela central única.
     Acesso direto (link antigo) é remanejado; dentro da central abre normal. */
  (function () {
    var PORTAL = 'https://borgesmacedoadvocacia.github.io/';
    if (location.hostname === 'localhost' || location.hostname === '127.0.0.1') return;
    if (window.self === window.top) { location.replace(PORTAL); return; }
    var anc = location.ancestorOrigins;
    if (anc && anc.length && anc[anc.length - 1] !== 'https://borgesmacedoadvocacia.github.io') {
      try { window.top.location.replace(PORTAL); } catch (e) { location.replace(PORTAL); }
    }
  })();
  <\/script>
